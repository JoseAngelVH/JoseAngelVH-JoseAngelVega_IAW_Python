...
def guardar_excel():
    ...
    # Cabecera
    ws.append(["Nombre"] + [f"Edad {i+1}" for i in range(10)])
    
    # Datos
    for u in usuarios:
        ws.append([u["nombre"]] + u["edades"])
...
def terminar():
    if usuarios:
        guardar_excel()
        messagebox.showinfo("Excel", "Datos guardados en usuarios.xlsx")
    else:
        messagebox.showinfo("Excel", "No hay datos para guardar")
